...
testthat::test_that("plots have known output", {
  barplot_esp_density_family <- function() barplot_pampa.f(
    agregation = "espece",
    metrique = "density",
    factGraph = "family",
  ...
  vdiffr::expect_doppelganger("barplot-especes-density-family", barplot_esp_density_family)

  barplot_unitobs_sr_family <- function() barplot_pampa.f(
    agregation = "unitobs",
    ...
    tableMetrique = "unit",
    new_window = FALSE, dataEnv = .dataEnv, baseEnv = .baseEnv)
  vdiffr::expect_doppelganger("barplot-unitobs-species_richness-family", barplot_unitobs_sr_family)
})
